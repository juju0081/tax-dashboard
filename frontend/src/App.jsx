...
// --- THIS POINTS TO YOUR CLOUD BRAIN ---
const API_BASE_URL = 'https://tax-dashboard-kb4k.onrender.com';
...
export default function App() {
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      alert('Error connecting to scanner in the cloud.');
    }
    setIsScanning(false);
    e.target.value = ''; 
  };
            ...
            <tbody>
              {receipts.map(r => (
                <tr key={r.id} style={{ borderBottom: '1px solid #444', backgroundColor: r.type === 'sell' ? '#451a1a' : '#172554' }}>
                  <td style={{ padding: '8px', fontWeight: 'bold', color: r.type === 'sell' ? '#f87171' : '#60a5fa' }}>{r.type.toUpperCase()}</td>
